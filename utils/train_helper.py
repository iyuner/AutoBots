...
def nll_pytorch_dist_joint(pred, data, agents_masks):
    ...
    loss = torch.nan_to_num(((-biv_gauss_dist.log_prob(data).sum(-1) * agents_masks).sum(2)) / num_active_agents_per_timestep).sum(1)
    return loss


def nll_loss_multimodes_joint(pred, ego_data, agents_data, mode_probs, entropy_weight=1.0, kl_weight=1.0,
                              use_FDEADE_aux_loss=True, agent_types=None, predict_yaw=False, mask=None):
    """
    Args:
      pred: [c, T, B, M, 5]
      ego_data: [B, T, 5]
    ...
    # agents_masks = torch.cat((ego_data[:,:,-1].unsqueeze(-1), agents_data[:, :, :, -1]), dim=-1)
    if mask is not None:
        agents_masks = agents_masks * ~(mask.squeeze(-1)) # missing part is False
